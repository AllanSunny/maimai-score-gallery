...
5. From that same sheet, score records are archived into [JSON files](data/scores) that are read by the app.
   - For songs that do not yet exist in the [song catalog](data/song-catalog.json) or any [manual overrides](data/song-overrides.json), their information is retrieved and archived from a SEGA endpoint (set on `SEGA_CATALOG_URL`, currently points to https://maimai.sega.jp/data/maimai_songs.json).
