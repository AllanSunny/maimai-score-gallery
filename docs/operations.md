...
`data/song-overrides.json`, then rerun **Import New Scores**. Judgment correction
...
in `data/song-overrides.json`. Standalone entries require an explicit stable ID,
...
`data/chart-summaries.json`. Each chart records its play count,
