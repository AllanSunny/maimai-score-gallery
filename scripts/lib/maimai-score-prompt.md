...
- The lower circular touchscreen shows the played song and primary result. Read song title, artist, chart-type badge, difficulty, level, achievement, sync, rating, rating change, and overall judgment totals from this display.
...
- Read the sync badge from the lower touchscreen using only the allowed schema values. Return `Sync` for the generic SYNC PLAY badge; do not confuse it with `FS`, `FS+`, `FDX`, or `FDX+`. Return null when no sync badge is present.
