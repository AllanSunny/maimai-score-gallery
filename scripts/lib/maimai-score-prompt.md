...
- Read chart type from the label immediately above the song title beside the level number on the lower touchscreen. Read `でらっくす` as `DX` and `スタンダード` as `STD`; prefer this visible Japanese label over styling, colors, or assumptions. Return `DX`, `STD`, or `UTAGE`. Default to `DX` only when no chart-type label is identifiable. UTAGE is unsupported downstream and must still be identified rather than treated as DX.
